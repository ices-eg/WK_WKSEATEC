...
export class WidgetService {
  ...
  getWidgets(): Observable<Widget[]>{
    ...
    return this.http.get<Widget[]>('/get-widgets',httpOptions)
    .pipe(
   ...
   postWidget(widget:Widget):Observable<Widget>{
     console.log(widget);
    return this.http.post<Widget>('/post-widget',widget,httpOptions)
    .pipe(
  ...
  public saveWidget(item:Widget):Observable<Widget>{
    return this.http.post<any>('/save-widget',item,httpOptions)
    .pipe(
      tap(_=>console.log("posted")),
  ...
  public getSavedWidgets():Observable<Widget[]>{
    return this.http.get<Widget[]>('/get-saved-widgets',httpOptions)
    .pipe(
  ...
  public addViewWidget(item:Widget){
    return this.http.get<string>('/get-widget-url/',{params:{name:item.docker},headers:httpOptions.headers})
    .subscribe((res)=>{
